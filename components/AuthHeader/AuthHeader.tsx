...
const AuthHeader = () => {
  ...
  if (!revenueHeadFilter.some((head) => head.includes(user?.revenue_head_id?.dispaly_name))) {
    return links.filter((link: any) => link.linkName !== "Applications");
  }
  return links;
};

const filteredNavLinks = filterNavLinks(NAV_LINKS[user?.role ?? "admin"], user);
      ...
      >
        ...
        <Hide breakpoint='(max-width: 992px)'>
          <Flex as='nav' align='center' gap={[10, null, null, 12]} h='52px'>
            {user?.enforcer_type === "revenue_head" ? filteredNavLinks.map((link: any) => {
              return (
                <BigNavLink
                  key={link.url}
                  {...link}
